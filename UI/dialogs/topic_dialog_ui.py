...
from PyQt5.QtWidgets import QGridLayout, QVBoxLayout, QHBoxLayout, QDialog

from UI.interface_utils import create_layout, create_frame, create_label, create_button, create_field, create_checkbox, \
    create_spacer, Spacer
from UI.styles import WIDGET, HEADER_FRAME
...
class TopicDialogUI:
    def _setup_ui(self, dialog):
        ...
        self.header_layout = create_layout(QHBoxLayout, [15, 0, 0, 0], 15, self.header_frame)
        self.header_label = create_label('Edit Topic', self.header_frame, self.header_layout, min_size=[30, 30], max_size=[200, 30])
        self.header_layout.addItem(create_spacer(Spacer.HORIZONTAL))
        self.exit_button = create_button('x', self.header_frame, min_size=30, max_size=30, add_layout=self.header_layout)
        ...
        self.topic_label = create_label('Topic', self.main_frame, self.topic_layout, min_size=[0, 30], max_size=[16777215, 30])
        ...
        self.alias_label = create_label('Alias', self.main_frame, self.alias_layout, min_size=[0, 35], max_size=[16777215, 30])
        ...
        self.qos_field = create_field(self.main_frame, self.qos_color_layout, [1, 0, 1, 1])
        self.qos_field.setMinimumSize(QtCore.QSize(230, 30))
        self.qos_field.setMaximumSize(QtCore.QSize(200, 30))
        self.qos_label = create_label('Qos', self.main_frame, self.qos_color_layout, [0, 0, 1, 1])
        self.color_layout = create_layout(QHBoxLayout)
        self.color_label = create_label('Color', self.main_frame, self.color_layout)
        self.color_button = create_button('■', self.main_frame, min_size=30, max_size=30, add_layout=self.color_layout)
        self.qos_color_layout.addLayout(self.color_layout, 0, 3, 1, 1)
        self.color_field = create_field(self.main_frame, self.qos_color_layout, [1, 3, 1, 1])
        ...
        self.no_local_label = create_label('No Local Flag', self.main_frame, self.no_local_layout)
        ...
        self.retain_published_label = create_label('Retain as Published Flag', self.main_frame, self.retain_published_layout)
        self.retain_published_checkbox = create_checkbox(self.main_frame, self.retain_published_layout)
        ...
        self.retain_handling_label = create_label('Retain Handling', self.main_frame, self.retain_handling_layout)
        self.retain_handling_field = create_field(self.main_frame, self.retain_handling_layout)
